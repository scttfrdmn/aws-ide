...
func NewStopCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Stop a running QGIS instance.

This will stop the EC2 instance and close any active DCV port forwarding.
The instance can be restarted later with 'lens-qgis start'.`,
...
func runStop(instanceID string, hibernate bool) error {
	...
	if instance.TunnelPID > 0 {
		if err := killProcess(instance.TunnelPID); err != nil {
			...
			fmt.Printf("DCV port forwarding (PID %d) stopped\n", instance.TunnelPID)
